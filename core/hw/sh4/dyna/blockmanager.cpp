...
RuntimeBlockInfo* bm_GetBlock2(void* dynarec_code)
{
   ...
   printf("bm_GetBlock(%08lX) failed ..\n",reinterpret_cast<uint64_t>(dynarec_code));
   return 0;
}
...
void bm_AddBlock(RuntimeBlockInfo* blk)
{
	...
	{
		printf("DUP: %08X %08lX %08X %08lX\n", (*blkmap.find(blk))->addr,reinterpret_cast<uint64_t>((*blkmap.find(blk))->code),blk->addr,reinterpret_cast<uint64_t>(blk->code));
		verify(false);
	}
	blkmap.insert(blk);
...
void bm_WriteBlockMap(const string& file)
{
	...
	if (f)
	{
		...
		for (size_t i=0; i<all_blocks.size(); i++)
		{
			fprintf(f,"block: %d:%08X:%08lX:%d:%d:%d\n",all_blocks[i]->BlockType,all_blocks[i]->addr,reinterpret_cast<uint64_t>(all_blocks[i]->code),all_blocks[i]->host_code_size,all_blocks[i]->guest_cycles,all_blocks[i]->guest_opcodes);
			for(size_t j=0;j<all_blocks[i]->oplist.size();j++)
				fprintf(f,"\top: %zu:%hu:%s\n",j,all_blocks[i]->oplist[j].guest_offs,all_blocks[i]->oplist[j].dissasm().c_str());
		}
		fclose(f);
...
void sh4_jitsym(FILE* out)
{
	for (size_t i=0; i<all_blocks.size(); i++)
	{
		fprintf(out,"%08lx %d %08lX\n",reinterpret_cast<uint64_t>(all_blocks[i]->code),all_blocks[i]->host_code_size,(uintptr_t)all_blocks[i]->addr);
	}
}
...
static void bm_PrintTopBlocks(void)
{
	...
	{
		printf("Block %08X: %06lX, r: %u (c: %u, s: %u, h: %u) (r: %.2lf%%, c: %.2lf%%, h: %.2lf%%)\n",
			all_blocks[i]->addr, reinterpret_cast<uint64_t>(all_blocks[i]->code),all_blocks[i]->runs,
	...
	printf(" >-< %.2lf%% covered in top 1%% blocks\n",sel_hops/total_hops);
...
void print_blocks(void)
{
	...
	for (size_t i=0;i<all_blocks.size();i++)
	{
		...
		if (f)
		{
			fprintf(f,"block: %08lX\n",reinterpret_cast<uint64_t>(blk));
			fprintf(f,"addr: %08X\n",blk->addr);
			fprintf(f,"hash: %s\n",blk->hash());
			fprintf(f,"hash_rloc: %s\n",blk->hash(false,true));
			fprintf(f,"code: %08lX\n",reinterpret_cast<uint64_t>(blk->code));
			fprintf(f,"runs: %d\n",blk->runs);
			fprintf(f,"BlockType: %d\n",blk->BlockType);
			fprintf(f,"NextBlock: %08X\n",blk->NextBlock);
			fprintf(f,"BranchBlock: %08X\n",blk->BranchBlock);
			fprintf(f,"pNextBlock: %08lX\n",reinterpret_cast<uint64_t>(blk->pNextBlock));
			fprintf(f,"pBranchBlock: %08lX\n",reinterpret_cast<uint64_t>(blk->pBranchBlock));
			fprintf(f,"guest_cycles: %d\n",blk->guest_cycles);
			fprintf(f,"guest_opcodes: %d\n",blk->guest_opcodes);
			fprintf(f,"host_opcodes: %d\n",blk->host_opcodes);
			fprintf(f,"il_opcodes: %zu\n",static_cast<size_t>(blk->oplist.size()));
